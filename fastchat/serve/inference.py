...
def load_model(model_name, device, num_gpus, load_8bit=False, debug=False):
    if device == "cpu":
        ...
        if num_gpus == "auto":
            ...
            if num_gpus != 1:
                kwargs.update({
                    "device_map": "auto",
                    "max_memory": {i: "28GiB" for i in range(num_gpus)},
                })
            else:
                kwargs.update({
                    "device_map": {'':0}
    ...
    if (device == "cuda" and num_gpus == 1 and not load_8bit) or device == "mps":
        model.to(device)
...
def generate_stream_medgpt(model, tokenizer, params, device, context_len=2048, stream_interval=2):
